    ...
    <div className="flex items-center justify-center gap-0 mb-8">
      {steps.map((label, i) => (
        <div key={i} className="flex items-center">
          <div className={`flex items-center gap-1.5 text-xs font-medium ${i + 1 === step ? 'text-accent' : i + 1 < step ? 'app-mut' : 'app-mut2'}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${i + 1 === step ? 'bg-accent text-white' : i + 1 < step ? 'app-surface2 app-mut' : 'app-surface2 app-mut2'}`}>
              {i + 1 < step ? '✓' : i + 1}
            </div>
            <span className="hidden sm:inline">{label}</span>
          </div>
          {i < steps.length - 1 && <div className="w-8 h-px app-surface2 mx-2" />}
        </div>
      ))}
    </div>
  );
...
function Step1({ onNext }: { onNext: (bizId: number, slug: string) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-xs app-mut mb-1">Nombre del negocio *</label>
        <input
          required value={name} onChange={e => handleNameChange(e.target.value)}
          className="w-full app-surface2 border app-line rounded-xl px-3 py-2.5 text-sm app-ink placeholder-gray-600 outline-none focus:border-accent/60 focus:ring-1 focus:ring-accent/30 transition"
          placeholder="Ej: Restaurante La Mar"
        />
      </div>

      <div>
        <label className="block text-xs app-mut mb-1">Tipo de negocio</label>
        <select
          value={businessType} onChange={e => setBusinessType(e.target.value)}
          className="w-full app-surface2 border app-line rounded-xl px-3 py-2.5 text-sm app-ink outline-none focus:border-accent/60 transition"
        >
      ...
      <div>
        <label className="block text-xs app-mut mb-1">Descripción corta</label>
        <textarea
          value={description} onChange={e => setDescription(e.target.value)} rows={2}
          className="w-full app-surface2 border app-line rounded-xl px-3 py-2.5 text-sm app-ink placeholder-gray-600 outline-none focus:border-accent/60 focus:ring-1 focus:ring-accent/30 transition resize-none"
          placeholder="Ej: Cocina peruana de autor con mariscos frescos"
        />
      </div>

      <div>
        <label className="block text-xs app-mut mb-1">URL de tu carta</label>
        <div className="flex items-center app-surface2 border app-line rounded-xl overflow-hidden focus-within:border-accent/60 focus-within:ring-1 focus-within:ring-accent/30 transition">
          <span className="text-xs app-mut2 pl-3 shrink-0">menubot.cl/chat/</span>
          <input
      ...
      <button
        ...
        className="w-full bg-accent hover:bg-accent-lite disabled:opacity-50 text-white font-semibold rounded-xl py-2.5 text-sm transition-all"
      >
...
function Step2({ businessId, onNext }: { businessId: number; onNext: (count: number) => void }) {
  ...
  if (result) {
    return (
      <div className="text-center py-4">
        ...
        <h3 className="text-xl font-bold app-ink mb-2">{result.totalDishes} platos en total</h3>
        <p className="app-mut text-sm mb-1">Completeness inicial: <span className="text-accent font-semibold">{result.completeness}%</span></p>
        <p className="app-mut text-xs mb-6">Vamos a completar la información que falta.</p>
  ...
  return (
    <div className="space-y-4">
      ...
      />
      <button onClick={() => onNext(0)} className="w-full text-xs app-mut2 app-ink-hover transition py-1">
...
function Step3({ businessId, slug, onDone }: { businessId: number; slug: string; onDone: () => void }) {
  ...
  if (!loaded) {
    loadDishes();
    return <div className="text-center py-8 app-mut text-sm">Cargando platos...</div>;
  }
  ...
  if (dishes.length === 0 || current >= dishes.length) {
    return (
      <div className="text-center py-6">
        <div className="text-5xl mb-4">✅</div>
        <h3 className="text-xl font-bold app-ink mb-2">¡Carta completa!</h3>
        <p className="app-mut text-sm mb-6">Tu menú está listo para que tus clientes lo exploren.</p>
        <Link
          href={`/chat/${slug}`}
          target="_blank"
        ...
        <button onClick={onDone} className="w-full text-sm app-mut app-ink-hover py-1 transition">
          Ir al dashboard
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className="w-full app-surface2 rounded-full h-1.5 mb-4">
          <div className="bg-accent h-1.5 rounded-full transition-all" style={{ width: `${pct}%` }} />
        </div>
      </div>

      <div className="app-surface2 rounded-xl px-4 py-3 text-sm">
        <p className="text-accent text-xs font-semibold mb-0.5">Plato</p>
        <p className="app-ink font-medium">{dish.name}</p>
        {dish.category && <p className="app-mut text-xs">{dish.category}</p>}
      </div>

      {field === 'description' && (
        <div>
          <label className="block text-xs app-mut mb-1">¿Cómo describirías este plato?</label>
          <textarea
            rows={2} value={(answers.description as string) ?? ''}
            onChange={e => setAnswers({ description: e.target.value })}
            className="w-full app-surface2 border app-line rounded-xl px-3 py-2.5 text-sm app-ink placeholder-gray-600 outline-none focus:border-accent/60 transition resize-none"
      ...
      {field === 'price' && (
        <div>
          <label className="block text-xs app-mut mb-1">¿Cuál es el precio de <strong className="app-ink">{dish.name}</strong>?</label>
          <div className="flex items-center app-surface2 border app-line rounded-xl overflow-hidden focus-within:border-accent/60 transition">
            <span className="app-mut pl-3 text-sm">$</span>
            <input
              type="number" min="0" value={(answers.price as number) ?? ''}
              onChange={e => setAnswers({ price: parseInt(e.target.value) })}
              className="flex-1 bg-transparent px-2 py-2.5 text-sm app-ink outline-none"
              placeholder="8990"
            />
            <span className="app-mut2 pr-3 text-xs">CLP</span>
          </div>
        </div>
      )}
      ...
      {field === 'ingredients' && (
        <div>
          <label className="block text-xs app-mut mb-1">Ingredientes principales</label>
          <input
            type="text" value={(answers.ingredients as string) ?? ''}
            onChange={e => setAnswers({ ingredients: e.target.value })}
            className="w-full app-surface2 border app-line rounded-xl px-3 py-2.5 text-sm app-ink placeholder-gray-600 outline-none focus:border-accent/60 transition"
      ...
      {field === 'allergens' && (
        <div>
          <label className="block text-xs app-mut mb-2">¿Contiene alérgenos?</label>
            ...
            {ALLERGEN_OPTIONS.map(a => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-medium transition ${selected ? 'bg-accent text-white' : 'app-surface2 app-mut app-soft-hover'}`}
                >
                  {a}
                </button>
              );
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={skip} className="flex-none px-4 py-2.5 rounded-xl app-soft border app-line text-sm app-mut app-ink-hover transition">
          Saltar
        </button>
        <button
          onClick={save} disabled={saving}
          className="flex-1 bg-accent hover:bg-accent-lite disabled:opacity-50 text-white font-semibold rounded-xl py-2.5 text-sm transition"
        >
      ...
      <main className="flex-1 flex items-start justify-center px-5 py-10">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold app-ink mb-1">{TITLES[step - 1]}</h1>
            <p className="app-mut text-sm">{SUBTITLES[step - 1]}</p>
          </div>

          <div className="app-surface border app-line rounded-2xl p-6">
            {step === 1 && (
